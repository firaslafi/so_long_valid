...
/*   Updated: 2023/07/30 20:14:06 by flafi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_t	*put_image_to_map(mlx_t *mlx, mlx_image_t *imgt, char *path)
{
	...
	if (fd < 0) // to verify that file exists or not
		{
...
void	ft_print_map(t_map *map)
{
	...
	while (map->map[i])
	{
		...
		{
			if (map->map[i][j] == '1')
				mlx_image_to_window(map->mlx, map->var->wall_img, j * 32, i * 32);
			else if (map->map[i][j] == 'P')
				mlx_image_to_window(map->mlx, map->var->player_img, j * 32, i * 32);
			else if (map->map[i][j] == 'C')
				mlx_image_to_window(map->mlx, map->var->coin_img, j * 32, i * 32);
			else
				mlx_image_to_window(map->mlx, map->var->floor_img, j * 32, i * 32);
			if (map->map[i][j] == 'E' && map->c_count == 0)
				mlx_image_to_window(map->mlx, map->var->exit_img, j * 32, i * 32);
			j++;
		}
		i++;
	}
